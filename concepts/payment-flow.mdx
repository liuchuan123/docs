...
<Info>
  **请求参数包含：** 商户交易号、订单金额与币种、商品信息、客户信息、跳转地址
</Info>
...
<CardGroup cols={3}>
  <Card title="银行卡" icon="credit-card">
    Visa / Mastercard / AMEX 等国际卡组织
  </Card>
  <Card title="电子钱包" icon="wallet">
    Alipay / WeChat Pay 等数字钱包
  </Card>
  <Card title="银行转账" icon="bank">
    银行直连支付通道
  </Card>
</CardGroup>
...
支付渠道（Stripe、Antom 等）处理支付后，通过异步回调通知 PayMatrix：
...
商户也可以调用 `GET /api/merchant/order/payment/query/{transactionId}` 主动查询支付状态。
