...
<Note>
  第一次接入？建议按顺序阅读下方的「标准接入流程」，配合 [接入指引](/getting-started/quickstart) 按步骤操作。
</Note>
...
| 环境 | API 基础地址 |
| --- | --- |
| 生产 | `https://openapi.paymatrixpay.com/api` |
| 沙盒 | `https://api-sbx.paymetrixpay.com/api` |

<Tip>
  先在沙盒环境完成联调，确认无误后再切换到生产环境。沙盒**不产生真实资金流动**，可放心测试。
</Tip>
