---
title: "快速开始"
description: "了解从商户入驻到完成第一笔支付的核心流程"
---


本章节帮助你快速了解从入驻 PMP Payment 到完成第一笔支付的完整流程。

## 整体架构

PMP Payment 为商户提供以下核心服务：
...
| 环境     | OpenAPI 地址                         | 说明                       |
| -------- | ------------------------------------ | -------------------------- |
| 沙盒环境 | `https://sandbox-openapi.example.com/api` | 用于联调测试，无真实资金   |
| 生产环境 | `https://openapi.example.com/api`         | 正式环境，需审核后开通     |

OpenAPI 接口前缀：
- 支付接口：`/merchant/payment`
- 订单查询：`/merchant/order`
...
```bash
curl -X POST "https://sandbox-openapi.example.com/api/merchant/payment/create" \
...
```bash
curl -X GET "https://sandbox-openapi.example.com/api/merchant/order/payment/query/TXN20260101001" \
  -H "X-Merchant-Id: YOUR_MERCHANT_ID" \
  -H "X-Timestamp: 1737004800000" \
  -H "X-Nonce: a3f7b2c1d4e5" \
  -H "X-Signature: <RSA_SIGNATURE>"
